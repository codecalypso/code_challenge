...
#require_relative "./fixtures/math_domain.csv"
#require_relative './fixtures/student_math_scores.csv'
...
describe "LearningPath" do
   ...
   parsed_domains = {"K"=>["RF", "RL", "RI"],
    ...
    let(:parsed_scores){ScoreParser.new('./fixtures/student_math_scores.csv').scores}
    let(:parsed_domain){DomainParser.new('/fixtures/math_domain.csv').domains}
   ...
   it "is domain agnostic" do
     expect(alternate_domain).to include({"Addie Green"=>["1.ML","2.AD","2.DV","2.GM","3.ML"]})
   end
end
